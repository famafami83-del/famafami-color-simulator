...
const PARTS = [
  ...
  //     대표 말: 「얇아서 테두리나 날개로 쓰기엔 힘이 없어서.」 매트리스커버와 까닭이
  //     다르다 — 거기는 얇아서 안 쓰는 것이고, 여기는 **테두리를 잡아주지 못해서**다.
  { key:'pipP',     ko:'line(테두리)',     def:WHITE, su:[60,80],
    suWhy:'100수는 얇아서 테두리에 힘이 없습니다',
  ...
  { key:'wMat',     ko:'매트리스커버',     def:WHITE, su:[60,80], suWhy:'100수는 얇아서 쓰지 않습니다',
    dz:['wing'], grp:'mat' },
  //   ★ 날개도 테두리라 **100수를 쓰지 않는다** [대표, 2026-08-10]. line 과 같은 까닭이다.
  //     문구만 「테두리」가 아니라 「날개」로 적는다 — 이 디자인에서 손님이 고르는 자리
  //     이름이 날개다. 남의 부위 이름이 나오면 자기 이야기로 안 읽힌다.
  { key:'wWing',    ko:'날개(테두리)',     def:WHITE, su:[60,80],
    suWhy:'100수는 얇아서 날개에 힘이 없습니다',
 ...
 *{box-sizing:border-box}
 ...
 /* 이 카드에서 **다음에 무슨 일이 생기는지** 말하는 자리만 진하게 [대표, 2026-08-10].
    나머지는 이미 아는 말(예상 금액이라는 것)이고, 손님이 기다려야 할 것은 결제 링크다.
    아래 안내(.ordnote b)와 같은 방식이다 — 본문 글꼴은 400 한 벌뿐이라 <b> 만으로는
    브라우저가 억지로 굵게 그려 흐릿하게 번진다. 진짜 굵은 벌이 있는 제목 글꼴로 바꾸고
    글자색도 곁말 회색에서 본문 네이비로 올린다. 굵기만으로는 약하다. */
 .qfoot b{font-family:var(--head);font-weight:700;color:var(--fg)}
 ...
 .flow{display:flex;align-items:center;justify-content:flex-start;gap:4px;margin:10px 0 0}
...
${PRICE_READY ? `  <div class="card">
    ...
    <!-- 결제 링크 흐름에 맞춘 문구다 [대표, 2026-08-10]. 전에는 「제작 전 주문 내용을
         확인한 후 최종 금액을 안내드립니다」였는데, 이제 금액을 말로 안내하는 것이
         아니라 **결제 링크를 보낸다.** 금액 이야기는 이 카드에서 끝난다 —
         아래 안내는 「무엇을 눌러 어떻게 보내는지」만 말한다. -->
    <p class="qfoot">선택하신 옵션을 기준으로 계산된 예상 금액입니다.<br>
      주문 내용을 확인한 뒤 <b>결제 링크를 보내드립니다.</b></p>
    <p class="qnote" id="qNote"></p>
  </div>
` : ''}  <pre id="orderTxt"></pre>
  <!-- 대표가 준 문구 그대로다 [대표, 2026-08-07]. 「추가금」도 「안내용」도 쓰지 않는다.
       한 문장에 한 줄 — 넓은 화면이라고 이어 붙이지 않는다.
       ★ 폰에서는 한 문장이 그래도 두 줄로 꺾인다 [대표, 2026-08-10]. 꺾이는 자리를
         브라우저에 맡기면 「카카오톡이 열 / 립니다」처럼 말 가운데가 잘린다. 그래서
         **뜻이 끊기는 자리마다 우리가 먼저 줄을 바꾼다** — 한 줄이 한 토막이 되게. -->
  <p class="ordnote">
    ${PRICE_READY ? `선택하신 내용과 예상 금액을 확인해주세요.` : `선택하신 내용을 확인해주세요.`}<br>
    ${INQUIRY ? `아래 <b>「카톡으로 주문하기」</b>를 누르면<br>
    주문 내용이 <b>자동으로 복사</b>되고 카카오톡이 열립니다.<br>
    열린 <b>채팅창</b>을 길게 눌러 <b>「붙여넣기」</b> 후 보내주세요.`
    // 결제 링크 이야기는 **금액 카드에서 이미 했다** [대표, 2026-08-10] —
