...
const C = {
  ...
  k980:  '#252628',   // NO. 980  딥 블랙 60수        — 날개(테두리)
};
...
// 삥 카드는 **양면 카드와 사진도 색도 똑같이** 두고 테두리 색 하나만 바꾼다.
// 두 카드가 나란히 놓이므로, 다른 것이 하나뿐이어야 그것이 무엇인지 바로 보인다.
const PIP  = { ...BOTH, bothP:C.m939 };
...
const SHOTS = {
  ...
  // 날개형 — 사진이 따로다(base_wing.jpg 1200×1600). 이불은 아몬드 밀크, 매트리스커버는
  //   베링씨로 다른 카드와 맞추고, **날개만 딥 블랙**으로 드러낸다. 이 디자인에서
  //   다른 것이 날개이므로 그것이 안 보이면 카드를 둔 뜻이 없다.
  //   베개는 블루 헤이즈 — 크림 이불 위에서 날개의 검정이 또렷하게 갈린다.
  'card_wing.jpg':  { base:'base_wing.jpg', w:520, q:76,
    crop:{ left:150, top:560, width:900, height:900 },
    paint:{ wQuilt:C.q901, wPillow:C.p2006, wPillowB:C.p2006, wMat:C.m939, wWing:C.k980 } },
  // 삥 — 위와 같은 사진·같은 색. 테두리만 베링씨로 드러낸다.
  //   ★ 크롭만 더 당겨 잡는다. 양면 카드와 같은 자리로 두었더니 **삥이 1픽셀도 안 됐다** —
