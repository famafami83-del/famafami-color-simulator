...
// 색은 대표가 정한 것이다 [대표, 2026-08-06]. 팔레트에 실제로 있는 색을 쓴다 —
// 카드에만 있는 색을 칠하면 손님이 그 색을 찾다가 없다.
//   ★ **원단 번호는 적지 않는다** [대표, 2026-08-12]. 이 저장소는 공개라 번호를 적으면
//     경쟁사가 우리 원단표를 그대로 가져간다. 색은 「컬러명 + 몇수」로만 부른다.
const C = {
  melt:      '#f5f4ef',   // 멜트 아이스크림 60수   — 무지 이불 · 양면 뒷면 · 날개 매트리스커버
  neutral:   '#aeb2b1',   // 뉴트럴 그레이 80수     — 무지 매트리스커버
  bluehaze:  '#c2d1ca',   // 블루 헤이즈 100수      — 무지 베개
  magicdust: '#eadbba',   // 매직 더스트 100수      — 무지 베개
  cherry:    '#f2d0ce',   // 체리 블라섬 100수      — 무지 베개(뒤)-왼쪽 [대표, 2026-08-10]
  misted:    '#eac070',   // 미스티드 옐로우 100수  — 무지 베개(뒤)-오른쪽 [대표, 2026-08-10]
  applelime: '#e2e7b1',   // 애플 라임 60수         — 양면 앞면
  almond:    '#f1ebdb',   // 아몬드 밀크 80수       — 양면 매트리스커버
  whitecap:  '#fbf1dd',   // 화이트 캡 100수        — line 이불 앞뒤 · 날개 바탕
  ebony:     '#23272a',   // 에보니 블랙 80수       — line 테두리
  mirage:    '#cdcec9',   // 미라지 그레이 80수     — line 매트리스커버
  forest:    '#587056',   // 포레스트 그린 60수     — 날개(테두리)
  // 2026-08-10 이전에 쓰던 색 — 되돌릴 때를 위해 남긴다.
  //   베링씨 #556378 · 아쿠아 에스케 #c1dee6 · 카키 그레이지 #c1beab · 딥 블랙 #252628
};
// 무지 카드의 베개 네 칸은 일부러 섞는다 — **한 장씩 따로 고를 수 있다**는 것이
// 카드에서부터 보여야 한다 [대표, 2026-08-10]. 양면·line 카드에는 이 자유가 없다
// (베개가 이불을 따라간다).
//   ★ 대표가 주신 주문서에는 **앞 두 장만** 있었다 (앞-왼쪽 블루 헤이즈 · 앞-오른쪽 매직 더스트).
//     주문서에는 **사신 장수만** 나오므로 뒤 베개 둘은 거기 없다. 뒤 두 자리는 대표가
//     따로 짚어 주셨다 [2026-08-10] — 뒤-왼쪽 체리 블라섬, 뒤-오른쪽 미스티드 옐로우.
...
const PILLOWS = {
  pillowL: C.bluehaze,    // 앞줄 왼쪽   — 블루 헤이즈
  pillowF: C.magicdust,   // 앞줄 오른쪽 — 매직 더스트
  pillowW: C.cherry,      // 뒷줄 왼쪽   — 체리 블라섬
  pillowR: C.misted,      // 뒷줄 오른쪽 — 미스티드 옐로우
};

// 양면 — 앞면 애플 라임, 뒷면 멜트 아이스크림, 매트리스 아몬드 밀크 [대표, 2026-08-10].
//   삥 자리는 앞면 색으로 덮어 안 보이게 한다. 페이지에서도 그렇게 쓴다.
const BOTH = { bothA:C.applelime, bothB:C.melt, bothM:C.almond, bothP:C.applelime };
...
const PIP  = { bothA:C.whitecap, bothB:C.whitecap, bothM:C.mirage, bothP:C.ebony };
...
const SHOTS = {
  ...
  'card_plain.jpg': { base:'base.jpg', w:520, q:76,
    ...
    paint:{ quilt:C.melt, mattress:C.neutral, ...PILLOWS } },
  ...
  //   ★ 색은 **대표가 하나씩 짚어 정한 것이다** [대표, 2026-08-10] — 이불·베개 바탕
  //     화이트 캡, 날개 포레스트 그린, 매트리스커버 멜트 아이스크림.
  //     전에는 이불 아몬드 밀크 · 베개 블루 헤이즈 · 매트리스 베링씨 · 날개 딥 블랙이었다.
  ...
  'card_wing.jpg':  { base:'base_wing.jpg', w:520, q:76,
    ...
    paint:{ wQuilt:C.whitecap, wPillow:C.whitecap, wPillowB:C.whitecap, wMat:C.melt, wWing:C.forest } },
